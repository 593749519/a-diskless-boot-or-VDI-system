...
1. service
   ...
   > ltservice.exe  -i  ;安装服务端服务
   >
   > ltservice.exe -u ;安装服务端服务

2. client

   本目录是客户端，主程序是ltclient.exe，通常它是被安装在镜像中

   > ltclient.exe  -i  ;安装客户端服务
   >
   > ltclient.exe -u ;卸载客户端服务
   >
   > ltclient.exe -d ;安装客户端驱动
...
3. imagetool
   ...
   其中bios_uefi_compatible_flag表示是否制作BIOS/UEFI兼容镜像
...
1. service
   ...
   > ltservice.exe  -i  ;Install the service
   >
   > ltservice.exe -u ;Uninstall the service
...
2. client
   ...
   > ltclient.exe  -i  ;Install the client service
   >
   > ltclient.exe -u ;Uninstall the client service
   >
   > ltclient.exe -d ;Install the client driver
...
3. imagetool
   ...
   The bios_uefi_compatible_flag means whether create a BIOS/UEFI compatible image
